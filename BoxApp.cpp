...
void BoxApp::Camera(const GameTimer& gt)
{
	...
	{
		if (gameObject.GetOpaqueItems()[i]->Type == "player" && moveUpPlayer) {
			gameObject.GetOpaqueItems()[i]->World._42 += 0.05;
			moveUpPlayer = false;
		}
		if (gameObject.GetOpaqueItems()[i]->Type == "player" && moveDownPlayer) {
			gameObject.GetOpaqueItems()[i]->World._42 -= 0.05;
